...
const searchProductsQuery = (whereClause, orderClause) =>
  `SELECT product.id, product.name, product.won_price, product.en_price, product.image, 
          COUNT(review.id) as reviews, CAST(AVG(review.rating) AS FLOAT) as rating ,
          CASE WHEN user_heart.product_id IS NOT NULL THEN TRUE ELSE FALSE END is_heart
   FROM product 
   LEFT JOIN
    review ON product.id = review.product_id 
    LEFT JOIN user_heart ON product.id=user_heart.product_id AND user_heart.user_id=? 
   ${whereClause} 
   GROUP BY 
   product.id 
   ${orderClause} 
