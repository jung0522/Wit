...
import {
  getPopularProductsByCategoryQuery,
  getPopularProductsByALLCategoryQuery,
  getNyamRecommendQuery,
} from './productQuery.js';
...
export const getALLProductByALLCategory = async (count) => {
  ...
  try {
    ...
    for (let category of categories) {
      ...
      if (category === 'ALL') {
        // 전체 카테고리 인기 상품
        [rows] = await pool.query(getPopularProductsByALLCategoryQuery, [
          parseInt(count, 10),
        ]);
      } else {
        // 특정 카테고리 인기 상품
        ...
        [rows] = await pool.query(getPopularProductsByCategoryQuery, [
          categoryId,
          parseInt(count, 10),
        ]);
      }
