...
router.get('/:productId', async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    return res.status(500).json(errResponse(err));
  }
});
...
router.get('/:productId/reviews', async (req, res, next) => {
  const { productId } = req.params;
  const { userId, cursor = 0, sort = 'latest', limit = 10 } = req.query;
  ...
  try {
    ...
    const [reviews] = await pool.query(reviewQuery, [userId, ...params]);
...
// 리뷰 작성하기 -> 이미지 업로드 통합
router.post('/:productId/reviews', imageUploader.single('image'), async (req, res, next) => {
  const { productId } = req.params;
...
router.get('/:productId/reviews/new', async (req, res, next) => {
  ...
  try {
    ...
    return res.status(500).json({
      isSuccess: false,
      code: 500,
      message: '리뷰 작성 페이지 불러오기 중 오류가 발생했습니다.',
    });
  }
});
...
router.get('/:productId/rating-stats', async (req, res, next) => {
  ...
  try {
    ...
    return res.status(500).json({
      isSuccess: false,
      code: 500,
      message: '별점 통계 불러오기 중 오류가 발생했습니다.',
    });
  }
});
...
router.post('/:productId/reviews/:reviewId/helpful', async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    return res.status(500).json({
      message: '리뷰 도움됨 표시 중 오류가 발생했습니다.',
    });
  }
});
